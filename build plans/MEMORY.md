...
- [Sprint 8 — IN PROGRESS](project_sprint8_state.md) — App store ready: TestFlight, Play Store, Sentry ✓, PostHog ✓, RLS ✓, awaiting Railway deployment
...
- [Sprint 8 current state](project_sprint8_state.md) — Detailed progress tracking, environment vars needed, parallel work possible

## Current state summary (Updated Apr 12, 2026)

### Sprint Status
- **Sprints complete:** 1, 2, 3, 4, 5, 6, 7 (fully complete — API + mobile)
- **Current:** Sprint 8 (App store ready) — 75% complete
  - ✅ Sentry error reporting (API + mobile)
  - ✅ PostHog analytics (17 typed events)
  - ✅ Privacy Policy + TOS (GitHub Pages hosted)
  - ✅ EAS Build configuration
  - ✅ Supabase Pro upgrade + RLS enabled
  - ⏳ API deployment to Railway (next)
  - ⏳ Screenshots for stores (parallel task)
  - ⏳ TestFlight + Play Store submission (blocked on API)

### Production Infrastructure
- **Database:** Supabase Pro (upgraded Apr 12)
  - RLS enabled on 30 tables, 35+ policies deployed
  - Daily backups, point-in-time recovery
  - User isolation: each user can only access own data
- **API base URL:** TBD (awaiting Railway deployment)
  - Will use `https://api.savvyspoon.app` (after domain purchase)
  - Temporary: Railway auto-generated domain
- **Error tracking:** Sentry (configured, ready for production build)
- **Analytics:** PostHog (configured, ready for production build)
- **Auth:** Supabase mobile SDK; token from `supabase.auth.getSession()` on every request via `apiClient.ts`
- **Pricing model:** Community crowdsourced only. `CanonicalPrice` fed by `PriceObservation` scans.
- **Stores:** All TX grocery chains (25+) via static `TX_GROCERY_STORES` array
- **Tiers:** Free / Plus ($4.99/mo) / Pro ($9.99/mo). New users get 7-day Pro trial on signup.
- **AI model config:** `src/lib/aiConfig.ts` — Haiku for price/reminder suggestions, Sonnet for meal plans.
